...
static auto tp_sac_hollow_bones_detail_get(Gamep g, Levelsp v, Levelp l, Thingp me) -> std::string
{
  TRACE();

  return                                                                                                                                //
      UI_INFO1_FMT_STR "You have hollow bones land with the touch of a feather upon falling into a chasm; which never happens, right? " //
      UI_INFO2_FMT_STR "Conversely, all other blunt trauma damage types are doubled in impact.\n";
}

static bool tp_sac_hollow_bones_on_damage(Gamep g, Levelsp v, Levelp l, Thingp me, ThingEvent &e)
{
  TRACE();
...
[[nodiscard]] auto tp_load_sac_hollow_bones() -> bool
{
  TRACE();

  auto *tp   = tp_load("sac_hollow_bones"); // keep as string for scripts
  auto  name = tp_name(tp);

  // begin sort marker1 {
  thing_detail_set(tp, tp_sac_hollow_bones_detail_get);
  thing_on_damage_set(tp, tp_sac_hollow_bones_on_damage);
