...
static const std::string option_1_targeted                     = "targeted blast strike";
...
static auto tp_spell_blast_strike_get(Gamep g, Levelsp v, Levelp l, Thingp me) -> std::string
{
  TRACE();
...
static bool tp_spell_blast_strike_on_cast_request(Gamep g, Levelsp v, Levelp l, Thingp spell, Thingp user, ThingEvent &e)
{
  TRACE();
...
static void tp_spell_blast_strike_spawn_explosion(Gamep g, Levelsp v, Levelp l, Thingp spell, Thingp user, bpoint p, ThingEvent &e)
{
  TRACE();
...
static bool tp_spell_blast_strike_on_cast_do(Gamep g, Levelsp v, Levelp l, Thingp spell, Thingp user, ThingEvent &e)
{
  TRACE();
      ...
      for (auto dy = -spell_radius; dy <= spell_radius; dy++) {
        ...
        if (! is_oob(p)) {
          if (distance(p, target) <= spell_radius) {
            tp_spell_blast_strike_spawn_explosion(g, v, l, spell, user, p, e);
          }
        }
      }
    }
...
static auto tp_spell_blast_strike_on_upgrade_possible(Gamep g, Levelsp v, Levelp l, Thingp me, TpSpellUpgrade u) -> bool
{
  TRACE();
...
static auto tp_spell_blast_strike_on_upgrade_do(Gamep g, Levelsp v, Levelp l, Thingp me, TpSpellUpgrade u) -> bool
{
  TRACE();
...
[[nodiscard]] auto tp_load_spell_blast_strike() -> bool
{
  TRACE();

  auto *tp   = tp_load("spell_blast_strike"); // keep as string for scripts
  auto  name = tp_name(tp);

  // begin sort marker1 {
  thing_detail_set(tp, tp_spell_blast_strike_get);
  thing_on_cast_do_set(tp, tp_spell_blast_strike_on_cast_do);
  thing_on_cast_request_set(tp, tp_spell_blast_strike_on_cast_request);
  thing_on_upgrade_do_set(tp, tp_spell_blast_strike_on_upgrade_do);
  thing_on_upgrade_possible_set(tp, tp_spell_blast_strike_on_upgrade_possible);
  tp_flag_set(tp, is_loggable);
  tp_flag_set(tp, is_spell);
  tp_name_long_set(tp, "blast strike");
