...
static void tp_spell_wildfire_storm_spawn_fire(Gamep g, Levelsp v, Levelp l, Thingp spell, Thingp user, bpoint p, ThingEvent &e)
{
  TRACE();
      ...
      for (auto dy = -spell_radius; dy <= spell_radius; dy++) {
        ...
        if (! is_oob(p)) {
          if (distance(p, target) <= spell_radius) {
            tp_spell_wildfire_storm_spawn_fire(g, v, l, spell, user, p, e);
          }
        }
      }
    }
