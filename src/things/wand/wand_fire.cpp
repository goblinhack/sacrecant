...
static auto tp_wand_fire_description_get(Gamep g, Levelsp v, Levelp l, Thingp me) -> std::string
{
  TRACE();
...
static auto tp_wand_fire_detail_get(Gamep g, Levelsp v, Levelp l, Thingp me) -> std::string
{
  TRACE();
...
[[nodiscard]] static auto tp_wand_fire_get_weapon_list(Gamep g, Levelsp v, Levelp l, Thingp me) -> std::vector< Tpp >
{
  TRACE();
...
[[nodiscard]] auto tp_load_wand_fire() -> bool
{
  ...
  auto *tp   = tp_load("wand_fire"); // keep as string for scripts
  auto  name = tp_name(tp);

  // begin sort marker1 {
  thing_description_set(tp, tp_wand_fire_description_get);
  thing_detail_set(tp, tp_wand_fire_detail_get);
  thing_get_weapon_list_set(tp, tp_wand_fire_get_weapon_list);
  ...
  tp_name_a_or_an_set(tp, "a wand of fire");
  tp_name_apostrophize_set(tp, "wand of fire's");
  tp_name_long_set(tp, "wand of fire");
