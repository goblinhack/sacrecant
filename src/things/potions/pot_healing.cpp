...
static auto tp_pot_healing_description_get(Gamep g, Levelsp v, Levelp l, Thingp me) -> std::string
{
  TRACE();
...
static auto tp_pot_healing_detail_get(Gamep g, Levelsp v, Levelp l, Thingp me) -> std::string
{
  TRACE();
...
static bool tp_pot_healing_on_use(Gamep g, Levelsp v, Levelp l, Thingp me, Thingp user)
{
  TRACE();
...
static void tp_pot_healing_on_death(Gamep g, Levelsp v, Levelp l, Thingp me, ThingEvent &e)
{
  TRACE();
...
[[nodiscard]] auto tp_load_pot_healing() -> bool
{
  TRACE();

  auto *tp   = tp_load("pot_healing"); // keep as string for scripts
  auto  name = tp_name(tp);

  // begin sort marker1 {
  thing_description_set(tp, tp_pot_healing_description_get);
  thing_detail_set(tp, tp_pot_healing_detail_get);
  thing_on_death_set(tp, tp_pot_healing_on_death);
  thing_on_thrown_end_set(tp, tp_pot_healing_on_thrown_end);
  thing_on_use_set(tp, tp_pot_healing_on_use);
  ...
  tp_name_long_set(tp, "potion of healing");
  tp_name_pluralize_set(tp, "potions of healing");
  tp_name_short_set(tp, "potion, healing");
