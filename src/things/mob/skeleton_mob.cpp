...
static auto tp_skeleton_mob_description_get(Gamep g, Levelsp v, Levelp l, Thingp me) -> std::string
{
  TRACE();

  return "skeleton spawning grounds";
}

static auto tp_skeleton_mob_detail_get(Gamep g, Levelsp v, Levelp l, Thingp me) -> std::string
{
  TRACE();
...
static void tp_skeleton_mob_tick_begin(Gamep g, Levelsp v, Levelp l, Thingp me)
{
  TRACE();
  ...
  if (! level_is_monst_bool(g, v, l, thing_at(g, v, l, me))) {
    (void) thing_mob_spawn_a_minion(g, v, l, me, tp_first(is_skeleton));
  }
}

[[nodiscard]] auto tp_load_skeleton_mob() -> bool
{
  auto *tp   = tp_load("skeleton_mob"); // keep as string for scripts
  auto  name = tp_name(tp);

  // begin sort marker1 {
  thing_description_set(tp, tp_skeleton_mob_description_get);
  thing_detail_set(tp, tp_skeleton_mob_detail_get);
  thing_on_death_set(tp, tp_skeleton_mob_on_death);
  thing_on_tick_begin_set(tp, tp_skeleton_mob_tick_begin);
  ...
  tp_name_a_or_an_set(tp, "a skeleton mob");
  tp_name_apostrophize_set(tp, "skeleton mob'");
