...
{
  switch (stat) {
    case THING_STAT_NONE :         return false;
    case THING_STAT_ATT :          return false;
    case THING_STAT_DMG :          return false;
    case THING_STAT_DEF :          return false;
    case THING_STAT_STR :          return false;
    case THING_STAT_CON :          return false;
    case THING_STAT_INT :          return false;
...
[[nodiscard]] auto thing_stat_set(Gamep g, Levelsp v, Levelp l, Thingp me, ThingStatType stat, uint8_t val) -> uint8_t
{
  ...
  auto out = me->_stat[ stat ] = val;

  if (compiler_unused) {
    THING_DBG(g, v, l, me, "set: %d => %s", out, thing_stat_dbg_string(g, v, l, me, stat).c_str());
  }
...
[[nodiscard]] auto thing_stat_long_string(Gamep g, Levelsp v, Levelp l, Thingp me, ThingStatType stat) -> std::string
{
  ...
  auto              mod       = thing_stat_mod(g, v, l, me, stat);
  auto              mod_str   = thing_stat_mod_value_string(g, v, l, me, stat);
  std::string const stat_name = stat_to_name_long(stat);
  std::string       stat_str;
