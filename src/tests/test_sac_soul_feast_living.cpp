...
[[nodiscard]] static auto test_sac_soul_feast_living(Gamep g, Testp t) -> bool
{
  TEST_LOG(t, "begin");
  TRACE();
  ...
  TEST_ASSERT(t, thing_hook_add(g, v, l, player, tp_find_mand("sac_soul_feast")), "failed to add sacrifice");
  ...
  TEST_ASSERT(t, wid_console_find_text(g, "You hit the mantisman"), "did not find console text");

  TEST_ASSERT(t, thing_health(g, v, l, player) == 95, "expecting specific player health");
...
[[nodiscard]] auto test_load_sac_soul_feast_living() -> bool // NOLINT
{
  TRACE();

  Testp test = test_load("sac_soul_feast_living");

  test_callback_set(test, test_sac_soul_feast_living);
