...
[[nodiscard]] static auto test_player_engulfed_then_falls(Gamep g, Testp t) -> bool
{
  TEST_LOG(t, "begin");
  TRACE();
  ...
  std::string const expect1
      = "XXXXXXX"
        ...
        "X.....X"
        "XXXXXXX";
  std::string const expect2 // first level
      = "XXXXXXX"
        "X.....X"
        "X.....X"
        "X.....X"
  ...
  Levelp  l1       = nullptr;
  Levelp  l2       = nullptr;
  Levelsp v        = game_test_init(g, &l1, level_num, w, h, start.c_str(), overrides);
  ...
  // Find the engulfer
  //
  FOR_ALL_THINGS_AT(g, v, l1, it, thing_at(g, v, l1, player))
  {
  ...
  // Move into the chasm
  //
  TEST_ASSERT(t, thing_move_to(g, v, l1, monst, thing_at(g, v, l1, player) + bpoint(0, 1)), "failed to move monster");
  ...
  //
  // Check the engulfer and player are gone
  //
  level_dump(g, v, l1, w, h);
  TEST_PROGRESS(t);
  {
  ...
  //
  // Check the engulfer and player fell
  //
  level_dump(g, v, l2, w, h);
  TEST_PROGRESS(t);
  {
...
[[nodiscard]] auto test_load_player_engulfed_then_falls() -> bool // NOLINT
{
  TRACE();

  Testp test = test_load("player_engulfed_then_falls");

  test_callback_set(test, test_player_engulfed_then_falls);
