...
[[nodiscard]] static auto test_spell_wildfire_storm(Gamep g, Testp t) -> bool
{
  TEST_LOG(t, "begin");
  TRACE();
  ...
  spell = thing_spawn(g, v, l, tp_find_mand("spell_wildfire_storm"), thing_at(g, v, l, player));
  TEST_ASSERT(t, spell, "failed to spawn spell");

  TEST_ASSERT(t, thing_spellbook_add(g, v, l, spell, player), "failed to add spell");
  e.reason                 = "test_spell_wildfire_storm";
...
[[nodiscard]] auto test_load_spell_wildfire_storm() -> bool // NOLINT
{
  TRACE();

  Testp test = test_load("spell_wildfire_storm");

  test_callback_set(test, test_spell_wildfire_storm);
