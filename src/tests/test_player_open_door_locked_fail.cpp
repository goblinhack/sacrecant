...
[[nodiscard]] static auto test_player_open_door_locked_fail(Gamep g, Testp t) -> bool
{
  TEST_LOG(t, "begin");
  TRACE();
...
[[nodiscard]] auto test_player_open_door_locked_fail() -> bool // NOLINT
{
  TRACE();

  Testp test = test_load("player_open_door_locked_fail");

  test_callback_set(test, test_player_open_door_locked_fail);
