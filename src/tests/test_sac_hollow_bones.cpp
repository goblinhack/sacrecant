...
[[nodiscard]] static auto test_hollow_bones(Gamep g, Testp t) -> bool
{
  TEST_LOG(t, "begin");
  TRACE();
  ...
  TEST_ASSERT(t, thing_hook_add(g, v, l1, player, tp_find_mand("sac_hollow_bones")), "failed to add sacrifice");
...
[[nodiscard]] auto test_load_sac_hollow_bones() -> bool // NOLINT
{
  TRACE();

  Testp test = test_load("sac_hollow_bones");

  test_callback_set(test, test_hollow_bones);
