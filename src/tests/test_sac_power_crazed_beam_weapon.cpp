...
[[nodiscard]] static auto test_sac_power_crazed_beam_weapon(Gamep g, Testp t) -> bool
{
  TEST_LOG(t, "begin");
  TRACE();
  ...
  TEST_ASSERT(t, thing_hook_add(g, v, l, player, tp_find_mand("sac_power_crazed")), "failed to add sacrifice");
...
[[nodiscard]] auto test_load_sac_power_crazed_beam_weapon() -> bool // NOLINT
{
  TRACE();

  Testp test = test_load("sac_power_crazed_beam_weapon");

  test_callback_set(test, test_sac_power_crazed_beam_weapon);
