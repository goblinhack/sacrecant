...
[[nodiscard]] static auto test_pot_levit_player_spiderweb(Gamep g, Testp t) -> bool
{
  TEST_LOG(t, "begin");
  TRACE();
  ...
  //
  // Check player is not dead when over spiderweb. It should be popped off the level.
  //
  {
    level_dump(g, v, l, w, h);
    TEST_PROGRESS(t);
    {
      TRACE();
      TEST_LOG(t, "check player is not dead when in spiderweb");
...
[[nodiscard]] auto test_load_pot_levit_player_spiderweb() -> bool // NOLINT
{
  TRACE();
