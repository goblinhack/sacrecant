...
[[nodiscard]] static auto test_sac_soul_feast_undead(Gamep g, Testp t) -> bool
{
  TEST_LOG(t, "begin");
  TRACE();
  ...
  TEST_ASSERT(t, wid_console_find_text(g, "You hit the skeleton"), "did not find console text");

  TEST_ASSERT(t, thing_health(g, v, l, player) == 96, "expecting specific player health");
...
[[nodiscard]] auto test_load_sac_soul_feast_undead() -> bool // NOLINT
{
  TRACE();

  Testp test = test_load("sac_soul_feast_undead");

  test_callback_set(test, test_sac_soul_feast_undead);
