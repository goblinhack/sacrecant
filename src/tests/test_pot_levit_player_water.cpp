...
[[nodiscard]] static auto test_pot_levit_player_water(Gamep g, Testp t) -> bool
{
  TEST_LOG(t, "begin");
  TRACE();
  ...
  //
  // Move into the water. The player should die.
  //
  {
    TEST_LOG(t, "move player into water right");
    TRACE();
  ...
  //
  {
    ...
    {
      TRACE();
      TEST_LOG(t, "check player is not submerged");
      p               = thing_at(g, v, l, player);
      found_submerged = false;

      FOR_ALL_THINGS_AT(g, v, l, it, p)
      {
        if (thing_is_player(it) && thing_is_submerged(g, v, l, it)) {
      ...
      if (found_submerged) {
        TEST_FAILED(t, "found the player submerged");
        goto exit;
      }
    }
  }
...
[[nodiscard]] auto test_load_pot_levit_player_water() -> bool // NOLINT
{
  TRACE();

  Testp test = test_load("pot_levit_player_water");

  // begin sort marker1 {
  test_callback_set(test, test_pot_levit_player_water);
  // end sort marker1 }
