...
[[nodiscard]] static auto test_sac_blood_pact(Gamep g, Testp t) -> bool
{
  TEST_LOG(t, "begin");
  TRACE();
  ...
  TEST_ASSERT(t, thing_health(g, v, l, player) == 96, "did not find expected health drop");

  TEST_ASSERT(t, thing_stamina(g, v, l, player) == 98, "did not find expected stamina drop");
...
[[nodiscard]] auto test_load_sac_blood_pact() -> bool // NOLINT
{
  TRACE();

  Testp test = test_load("sac_blood_pact");

  test_callback_set(test, test_sac_blood_pact);
