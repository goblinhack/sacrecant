...
auto operator<<(std::ostream &out, Bits< const Config & > const my) -> std::ostream &
{
  ...
  //
  {
    ...
    out << bits(tmp);
    tmp = offsetof(Config, key_cast);
    out << bits(tmp);
    tmp = offsetof(Config, key_spell1);
    out << bits(tmp);
    tmp = offsetof(Config, key_spell2);
    out << bits(tmp);
    tmp = offsetof(Config, key_spell3);
    out << bits(tmp);
    tmp = offsetof(Config, key_spell4);
    out << bits(tmp);
    tmp = offsetof(Config, key_spell5);
    out << bits(tmp);
    tmp = offsetof(Config, key_spell6);
    out << bits(tmp);
    tmp = offsetof(Config, key_spell7);
    out << bits(tmp);
    tmp = offsetof(Config, key_spell8);
    out << bits(tmp);
    tmp = offsetof(Config, key_spell9);
    out << bits(tmp);
    tmp = offsetof(Config, key_spell10);
    out << bits(tmp);
    tmp = offsetof(Config, key_spell11);
    out << bits(tmp);
    tmp = offsetof(Config, key_spell12);
    out << bits(tmp);
    tmp = offsetof(Config, key_spell13);
    out << bits(tmp);
    tmp = offsetof(Config, key_spell14);
    out << bits(tmp);
    tmp = offsetof(Config, key_spell15);
    out << bits(tmp);
    tmp = offsetof(Config, key_spell16);
    out << bits(tmp);
    tmp = offsetof(Config, key_spell17);
    out << bits(tmp);
    tmp = offsetof(Config, key_spell18);
    out << bits(tmp);
    tmp = offsetof(Config, key_spell19);
    out << bits(tmp);
    tmp = offsetof(Config, key_spell20);
    out << bits(tmp);
  ...
  out << bits(my.t.key_spell10);
  out << bits(my.t.key_spell11);
  out << bits(my.t.key_spell12);
  out << bits(my.t.key_spell13);
  out << bits(my.t.key_spell14);
  out << bits(my.t.key_spell15);
  out << bits(my.t.key_spell16);
  out << bits(my.t.key_spell17);
  out << bits(my.t.key_spell18);
  out << bits(my.t.key_spell19);
  out << bits(my.t.key_spell20);
...
auto operator<<(std::ostream &out, Bits< const class Game & > const my) -> std::ostream &
{
  ...
  //
  {
    ...
    out << bits(tmp);                                     // all we're saving here is offsets for validation later
    tmp = offsetof(Thing, _distance_throw);               // all we're saving here is offsets for validation later
    out << bits(tmp);                                     // all we're saving here is offsets for validation later
    tmp = offsetof(Thing, _is_hit);                       // all we're saving here is offsets for validation later
    out << bits(tmp);                                     // all we're saving here is offsets for validation later
    tmp = offsetof(Thing, _is_hidden);                    // all we're saving here is offsets for validation later
    out << bits(tmp);                                     // all we're saving here is offsets for validation later
    ...
    out << bits(tmp);                                     // all we're saving here is offsets for validation later
    tmp = offsetof(Thing, _noise);                        // all we're saving here is offsets for validation later
    out << bits(tmp);                                     // all we're saving here is offsets for validation later
    ...
    out << bits(tmp);                                     // all we're saving here is offsets for validation later
    tmp = offsetof(Thing, _spell_mana_cost);              // all we're saving here is offsets for validation later
    out << bits(tmp);                                     // all we're saving here is offsets for validation later
    tmp = offsetof(Thing, _spell_radius);                 // all we're saving here is offsets for validation later
    out << bits(tmp);                                     // all we're saving here is offsets for validation later
    tmp = offsetof(Thing, _spell_range);                  // all we're saving here is offsets for validation later
    out << bits(tmp);                                     // all we're saving here is offsets for validation later
    tmp = offsetof(Thing, _spell_cost);                   // all we're saving here is offsets for validation later
    out << bits(tmp);                                     // all we're saving here is offsets for validation later
    ...
    out << bits(tmp);                                     // all we're saving here is offsets for validation later
    tmp = offsetof(Thing, _mana);                         // all we're saving here is offsets for validation later
    out << bits(tmp);                                     // all we're saving here is offsets for validation later
    tmp = offsetof(Thing, _sac_points_max);               // all we're saving here is offsets for validation later
    out << bits(tmp);                                     // all we're saving here is offsets for validation later
    tmp = offsetof(Thing, _sac_points);                   // all we're saving here is offsets for validation later
    out << bits(tmp);                                     // all we're saving here is offsets for validation later
    tmp = offsetof(Thing, _jump_attack_pct_chance);       // all we're saving here is offsets for validation later
    out << bits(tmp);                                     // all we're saving here is offsets for validation later
    tmp = offsetof(Thing, _items_collected_max);          // all we're saving here is offsets for validation later
    out << bits(tmp);                                     // all we're saving here is offsets for validation later
    tmp = offsetof(Thing, _attack_count_per_tick);        // all we're saving here is offsets for validation later
    out << bits(tmp);                                     // all we're saving here is offsets for validation later
    tmp = offsetof(Thing, _hearing_threshold);            // all we're saving here is offsets for validation later
    out << bits(tmp);                                     // all we're saving here is offsets for validation later
    tmp = offsetof(Thing, _distance_throw);               // all we're saving here is offsets for validation later
    out << bits(tmp);                                     // all we're saving here is offsets for validation later
    tmp = offsetof(Thing, _charge_count);                 // all we're saving here is offsets for validation later
    out << bits(tmp);                                     // all we're saving here is offsets for validation later
    tmp = offsetof(Thing, _distance_avoid_target);        // all we're saving here is offsets for validation later
    out << bits(tmp);                                     // all we're saving here is offsets for validation later
    ...
    out << bits(tmp);                                     // all we're saving here is offsets for validation later
    tmp = offsetof(Thing, _dormant);                      // all we're saving here is offsets for validation later
    out << bits(tmp);                                     // all we're saving here is offsets for validation later
